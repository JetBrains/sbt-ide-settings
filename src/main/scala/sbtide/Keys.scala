...
object Keys {
  ...
  lazy val ideExcludePaths = SettingKey[Classpath]("ide-exclude-paths",
  ...
  lazy val ideDefaultSettings = Seq(
    ...
    ideExcludePaths := (target.value ** Util.AncestorFilter(sourceManaged.value, resourceManaged.value)).classpath
  )
}
